...
namespace PasteSharp
{
	public class Fpaste : Bridge
	{
		...
		public void SendContentRequest(string content, string nick, string language ,string expiry) {
			...
			var match = Regex.Match(Session.Fpaste( content,  nick,  language , expiry), 
				@"""id"": ""([0-9a-zA-Z]+)""",RegexOptions.IgnoreCase);

			if (match.Success) {
			    GetUrlRequest = "http://fpaste.org/" + match.Groups [1].Value;
			}
		}
